...
### 软件架构
<img src="https://gitee.com/ch-cloud/wiki/raw/master/images/v2/ch-cloud.jpg" alt="软件架构"/>
...
Data ID: ch-sso.yml  
Group: ch-sso  
直接使用[nacos/ch-sso/ch-sso.yml](nacos/ch-sso/ch-sso.yml)  
注意：请需要修改数据库、Redis、rocketMQ连接及相应的账号和密码
...
docker run -d \
  ...
  --restart always \
  -p 8080:8080 \
  --env JAVA_OPTS="-Dnacos.config.server-addr=xxx:8848 \
                   -Dnacos.config.namespace=xxx \
                   -Dspring.cloud.nacos.username=nacos \
                   -Dspring.cloud.nacos.password=nacos \
                   -Dspring.profiles.active=test" \
  --pull always \
  chaohua2025/ch-sso:latest
```
...
Data ID: ch-upms.yml  
Group: ch-upms  
直接使用[nacos/ch-sso/ch-upms.yml](nacos/ch-upms/ch-upms.yml)  
注意：请需要修改数据库、Redis、rocketMQ连接及相应的账号和密码
